...
class Solution:
    ...
    def containsNearbyAlmostDuplicate1(self, nums, k, t):
        ...
        for i in range(len(nums)):
            a = nums[i] - t  # 如果record 中大于等于a的最小的元素 <= nums[i]+t,return True
            OrderedDict(sorted(record.items(), key=lambda t: t[0]))
            for r in record:
                if r >= a and r <= nums[i] + t:
                    return True
            record[nums[i]] = None
    ...
    def containsNearbyAlmostDuplicate3(self, nums, k, t):
        ...
        dic = OrderedDict()
        for n in nums:
            key = n if not t else n // t   #当t=0时，key=n, 当t != 0时，key=n//t
...
if __name__ == "__main__":
    ...
    #这个结果为True? why???????
    so = Solution()
    re = so.containsNearbyAlmostDuplicate2(nums, k ,t)
    pass
