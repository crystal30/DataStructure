...
class BST:
    ...
    # 找二分搜索树中小于等于e的最大元素
    def floor(self, e):
        return self.__floor(self.root, e)

    def __floor(self, Node, e):

        if Node == None:
            return None
        else:
            if Node.e <= e:
                if Node.right != None and Node.right <= e:
                    return self.__floor(Node.right, e)
                return Node.e
            else:
                return self.__floor(Node.left, e)
...
if __name__ == '__main__':
    ...
    print('levelOrder')
    bst.levelOrder()
    print('mim',bst.minimum())
    print('max', bst.maximum())
    print('bst removeMax',bst.removeMax())
    print('inOrder')
    bst.inOrder()

    print('bst.removeMin',bst.removeMin())
    print('inOrder')
    bst.inOrder()
