...
class IndexMinHeap():
    ...
    def __slipUp(self,k):
        ...
        while parent >= 0:
            if self.__data[self.__indexs[k]] < self.__data[self.__indexs[parent]]:
    ...
    def extraMax(self):
        ...
        #当堆中只剩下一个元素时，只用将堆中的元素推出即可，不用再进行其它多余的操作
        if self.getSize() == 1:
            self.__indexs.pop()
        else:
    ...
    def __slipDown(self,p):
        while self.__leftChild(p)< self.getSize():
            ...
            if child+1 <self.getSize() and \
                    self.__data[self.__indexs[child+1]] < self.__data[self.__indexs[child]]:
                child += 1
            if self.__data[self.__indexs[child]] < self.__data[self.__indexs[p]]:
